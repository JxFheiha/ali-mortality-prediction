...
if __name__ == '__main__':
    file_path1 = "E:/ML/ALI-stroke/pythonML/202508revise/stroke-train-smote-transformed-data.xlsx"
    print(file_path1)
    df = pd.read_excel(file_path1)


    features1 = set(df.columns)
    features1.remove("Stroke")
    features1 = list(features1)
    X_train = np.array(df[features1])
    print(X_train.shape)
    y_train = df["Stroke"]
    #
    file_path2 = "E:/ML/ALI-stroke/pythonML/202508revise/stroke-test-transformed-data.xlsx"
    print(file_path2)
    df = pd.read_excel(file_path2)
    features2 = set(df.columns)
    features2.remove("Stroke")
...
def drawRoc(roc_auc,fpr,tpr):
    ...
    plt.title('Stroke-ROC Curve', fontsize=13)
    plt.legend(loc="lower right")
    plt.show()
    plt.savefig('Stroke-ROC_300dpinew.pdf', dpi=300)
